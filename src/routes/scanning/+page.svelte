...
{#if scanningState === 'caught' && currentDrop}
	...
	>
		...
		{/if}
		<img src={`items/${currentDrop.itemId}.webp`} alt={currentDrop.itemId} height="150" loading="lazy" />
	</button>
{/if}
...
{:else if scanningState === 'timeout'}
	<div class="animation">
		<LottiePlayer src="/animations/time2.json" loop autoplay width={192} />
	</div>
	<div class="scanning-action">{$t('message.timeout')}</div>
{:else if scanningState === 'waiting'}
	<div class="animation">
		<LottiePlayer src="/animations/scan-double.json" loop autoplay width={192} />
	</div>
