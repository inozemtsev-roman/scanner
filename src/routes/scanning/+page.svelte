<script lang="ts">
	...
	import type { FingerDrop } from '$lib/fingerDrop'
	...
	let currentDrop: FingerDrop | null = null
	...
	$: if (myData) {
		console.log(myData)
		const timeUntilCanFish = new Date(myData.lastTimeScanned ?? 0).getTime() + 30_000 - new Date().getTime()
		setTimeout(
			() => {
				scanningState = 'idle'
			},
...
{#if scanningState === 'caught' && currentDrop}
	...
	>
		...
		{/if}
		<img src={`items/${currentDrop.itemId}.webp`} alt={currentDrop.itemId} width="160" loading="eager" />
	</button>
{/if}
...
{:else if scanningState === 'timeout'}
	...
	<div class="scanning-action">Очистка завершена, приложите палец к сканеру</div>
{:else if scanningState === 'waiting'}
	<div class="animation">
		<LottiePlayer src="/animations/scan.json" loop autoplay width={192} />
	</div>
	<div class="scanning-action">Подтвердите отпечаток пальца</div>
{:else if scanningState === 'missed'}
	<div class="animation">
		<LottiePlayer src="/animations/rainCloud.json" loop autoplay width={192} />
	</div>
	<div class="scanning-action">Отпечаток не распознан</div>
{/if}
