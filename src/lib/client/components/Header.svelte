	...
	<button
		...
		on:click={() =>
			window.Telegram.WebApp.showPopup({
				...
				message: `You current level: ${myData.xp} / ${getXpForNextLevel(myData.level)} XP (Experience Points)`
			})}
	>
...
<style lang="scss">
	...
	.loading-indicator {
		width: 100%;
		height: 3px;
		background: linear-gradient(to right, var(#2188ff), var(--background), var(#2188ff));
		background-size: 200%;
		position: absolute;
		left: 0;
		bottom: 0;
